...
## Roster-sync lifecycle, concurrency, and freshness

Task 007B.2 treats each fully validated league users-and-rosters bundle as one observation. The exact account/provider/sport/season/league scope is frozen when the run starts. A valid running attempt and scope are reused for 15 minutes without source reads. An older attempt, or a fresh attempt with missing or inconsistent scope, is failed with bounded metadata, its private rows are cleaned, and a replacement may start.

One normalized bundle per expected league stages privately. Exact replay is idempotent, changed replay fails closed, progress is the distinct staged-league count, and each successful first stage advances `updated_at` as the heartbeat. Completion requires the staged external league set to equal the frozen set and publishes only after revalidating every staged bundle.

Shared `league_users`, `rosters`, and `roster_players` current state is monotonic by the per-league complete-bundle watermark `leagues.roster_bundle_fetched_at`, including current flags, owner and co-owner values, settings, exact nullable source arrays, keeper state, creation, reactivation, and removal. After the league lock, only an equal-or-newer fully validated users-and-rosters bundle may mutate shared state and advance the watermark. An older bundle skips every shared mutation and is counted separately, so stale inclusion cannot reverse newer absence; per-row guards remain defense in depth.

After applying or skipping shared state, the run resolves its account's ownership from current canonical active rosters at the stored league watermark. `fantasy_account_leagues.roster_ownership_status` and `roster_ownership_observed_at` record paired `owned`, `not_owned`, or `unresolved` truth and observation time. Exactly one match records `owned`; a known zero match records `not_owned`; a zero match with unknown current co-owner arrays records `unresolved` and preserves any earlier association as history. Resolution never moves backward in observation time and never mutates another fantasy account.

Concurrent first creation of a shared league user, roster, or membership must use conflict-safe insert-or-load behavior and continue with the canonical stored row. Catching and ignoring a unique violation is prohibited. Work and locks must follow one canonical sequence: external league ID, then external league-user ID, then external roster ID, then exact player ID.

Roster-sync removals are limited to the exact fantasy account, provider, sport, season, and league set in the validated complete collection. Shared removal requires an applied equal-or-newer complete bundle, and one account sync never removes another account's ownership association. Local-Supabase integration coverage retains the simultaneous same-resource race, adds a newer-absence/older-inclusion race, and proves newer `not_owned` and `unresolved` observations survive older positive bundles until later newer confirmation. Every scenario has a hard timeout. A separate 30-league, 360-roster, 7,200-membership load test holds atomic completion below its 60-second statement timeout and verifies the applied/stale bundle plus ownership-resolution counts.

`succeeded` means every league ends `owned` or `not_owned`. `partial` means the complete valid shared source collection published but one or more league ownership results ended `unresolved` because current canonical co-owner source state was unknown. Bounded results distinguish applied and stale-skipped shared league bundles, owned, confirmed-not-owned, unresolved, and stale-skipped ownership resolutions. Malformed or incomplete source fails the run, cleans private state, and preserves prior public rows. Roster import never updates `fantasy_accounts.last_synced_at`.
