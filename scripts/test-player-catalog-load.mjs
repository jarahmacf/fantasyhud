...
async function run() {
  ...
  const [players, primaryIds, allPrimaryMappings, run, sample] =
    await Promise.all([
      identityA.client
        .from("players")
  ...
  console.log(
    `Player catalog load test passed: ${recordCount} records, ${recordCount} players, ${recordCount} primary mappings, ${Date.now() - startedAt}ms.`
  )
}
