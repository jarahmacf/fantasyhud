...
Task 008A.2 introduces the empty shared draft-domain architecture: league and account draft-collection state, canonical drafts, normalized slots, explicit account participation, complete picks, historical player context, conservative draft-environment identity, and finalized-board protections. All five new public tables use indexed RLS and explicit safe authenticated projections. Shared board reads require active confirmed participation through a tracked account; negative, unresolved, or removed participation does not expose a board. Exact participant maps, picked-by IDs, external roster evidence, and raw metadata remain server-only. Browser roles and `service_role` receive no direct mutation privileges, and `service_role` receives no direct read privilege. `draft_sync` becomes an allowed observable scope with its own running uniqueness, but no source client, private staging, lifecycle RPC, Server Action, route, metric, or UI is added. Task 008B remains unstarted and will implement the reviewed complete-collection lifecycle.
...
The Task 008A.2 draft branch additionally defines `fantasy_account_draft_collections`, `drafts`, `draft_slots`, `fantasy_account_drafts`, and `draft_picks`; these remain empty architecture tables until Task 008B.

See `LEAGUE_DISCOVERY.md`, `PLAYER_CATALOG.md`, `ROSTER_DOMAIN.md`, and `ROSTER_IMPORT.md` for current source boundaries, `DRAFT_DOMAIN.md` for the draft schema and Task 008B contract, `FANTASY_DATA_ARCHITECTURE.md` for grains and history rules, `ADP_CONTEXT_ARCHITECTURE.md` for the future context-aware ADP contract, `PERFORMANCE_VS_DRAFT_CAPITAL_ARCHITECTURE.md` for future outcome and capital-efficiency boundaries, and `SYNC_ARCHITECTURE.md` for lifecycle, concurrency, and sanitized-error requirements. No current task adds a queue, cache, scheduler, complete portfolio timestamp, or production Supabase project.
