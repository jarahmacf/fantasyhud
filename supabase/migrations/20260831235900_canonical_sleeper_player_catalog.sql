...
comment on column public.provider_catalog_runs.triggered_by_user_id is
  'Server-only audit and run-ownership state intentionally excluded from browser grants.';
...
grant select (
  ...
  created_at,
  updated_at
) on table public.provider_catalog_runs to authenticated;
...
begin
  ...
  select
    ...
    pg_catalog.count(*) filter (
      where stage.normalized_record -> 'profile' ->> 'entity_type' = 'player'
        and (stage.normalized_record -> 'profile' ->> 'active')::boolean
    )::integer,
    pg_catalog.count(*) filter (
