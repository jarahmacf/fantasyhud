...
select is(
  ...
  16,
  'authenticated can select every sanitized catalog-run status column'
);
select ok(
  ...
  'authenticated cannot select the server-only triggering user ID'
);
...
select is(
  ...
  'the lifecycle retains the triggering user ID as private run ownership state'
);
select results_eq(
  $$
...
values
  ...
  (
    '72000000-0000-0000-0000-000000000092', 'nfl', 'team_defense',
    'Absent Defense', 'DEF', array['DEF'], true, 'Active', 'sleeper', '{}',
    '2026-08-30T12:00:00Z'
  );
...
values
  ...
  (
    '72000000-0000-0000-0000-000000000092', 'sleeper', 'nfl',
    'absent-defense', 'sleeper', true, '{}',
    '2026-08-01', '2026-08-02', null
  );
...
select is(
  (
    ...
    where player.entity_type = 'player'
      and player.active
  ...
  498,
  'the dashboard-style query counts current mapped active individual players'
);
select is(
  (
    select count(*)::integer
    ...
    inner join public.player_external_ids as external_id
      on external_id.player_id = player.id
    where player.entity_type = 'team_defense'
      and external_id.namespace = 'sleeper'
      and external_id.sport = 'nfl'
      and external_id.is_primary
      and external_id.removed_at is null
  ...
  1,
  'the dashboard-style query counts current mapped team defenses'
);
select is(
  (
    select count(*)::integer
    ...
    where player.entity_type = 'player'
      ...
      and external_id.is_primary
      and external_id.removed_at is null
      and external_id.external_id in ('p0011', 'p0012')
  ),
  0,
  'active defense and sparse unknown sentinels do not inflate active players'
);
select is(
  (
    select count(*)::integer
    ...
    inner join public.player_external_ids as external_id
      on external_id.player_id = player.id
    where player.id = '72000000-0000-0000-0000-000000000090'
      and player.entity_type = 'player'
      and player.active
      and external_id.namespace = 'sleeper'
      and external_id.sport = 'nfl'
      and external_id.is_primary
  ...
  0,
  'an active player with a removed primary mapping is not current'
);
select is(
  (
    select count(*)::integer
    ...
    inner join public.player_external_ids as external_id
      on external_id.player_id = player.id
    where player.id = '72000000-0000-0000-0000-000000000092'
      and player.entity_type = 'team_defense'
      and external_id.namespace = 'sleeper'
      and external_id.sport = 'nfl'
      and external_id.is_primary
      and external_id.removed_at is null
  ...
  0,
  'a team defense with a removed primary mapping is not current'
);
...
select lives_ok(
  $$
    select id, provider, sport, catalog, status, progress_current,
      progress_total, source_fetched_at, source_record_count, source_bytes,
      result_counts, error_summary, started_at, finished_at, created_at,
      updated_at
    from public.provider_catalog_runs
  $$,
  'authenticated users can query the complete safe catalog status projection'
);
select throws_ok(
  $$ select triggered_by_user_id from public.provider_catalog_runs $$,
  '42501',
  null,
  'authenticated selection of the private triggering user ID is denied'
);
select throws_ok(
  $$ delete from public.provider_catalog_runs $$,
  '42501',
  null,
  'authenticated users cannot mutate provider catalog runs'
);
select throws_ok(
